...
M5A = function(data, BadStart, nchains, method, niter, lambda.min, q.diff){
    ...
    m5a = function(params){
        if (params[1] < 0 | params[1] > 2*pi | params[2] <= 0 | params[2] > 227 | params[3] < 0 | params[3] > 2*pi | params[4] < lambda.min | params[4] > lambda.max){
            R = BadStart
            return(R)
        }
        ...
        if (min.q < q.diff){
        	...
        	P = circularp(data)
            R = dmixedvonmises(data, mu1 = as.circular(params[1], control.circular = P), mu2 = as.circular(params[3], control.circular = P), kappa1 = params[2], kappa2 = params[2], prop = params[4])
            R = -sum(log(R))
            return(R)
        }
    }
    ...
    min = which.min(sapply(m5a.out,function(x) x[2]))
    return(m5a.out[[min]])
}
