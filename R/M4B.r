...
M4B = function(data, BadStart, nchains, method, niter){
    ...
    m4b = function(params){
        if (params[1] < 0 | params[1] > 2*pi | params[2] <= 0 | params[2] > 227 | params[3] <= 0 | params[3] > 227 | params[4] < lambda.min | params[4] > lambda.max){
            ...
            R = dmixedvonmises(data, mu1 = circular(params[1]), mu2 = circular(params[1]+pi, modulo = "2pi"), kappa1 = params[2], kappa2 = params[3], prop = params[4])
            R = -sum(log(R))
            return(R)
        }
    }
